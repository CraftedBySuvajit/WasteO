...
const migrateDB = async () => {
  try {
    ...
    if (!mongoUri) {
      throw new Error('❌ MONGO_URI is not defined in .env file. Please check server/.env');
    }
    ...
    for (let i = 1; i <= 52; i++) {
      demoUsers.push({
        ...
        role: 'student',
        name: `Student User ${i}`,
        dept: i % 2 === 0 ? 'Computer Science' : 'Environmental Science',
        block: i % 3 === 0 ? 'B' : 'A',
        rewardPoints: 100 + (i * 5),
      });
    }
    ...
    const student = createdUsers.find(u => u.email === 'student@edu.in');
    const collector = createdUsers.find(u => u.email === 'collector@edu.in');
    ...
    console.log('─────────────────────────────');
    console.log('Login with these emails (Password: 12345678):');
    console.log('  Student:   student@edu.in');
    console.log('  Collector: collector@edu.in');
