...
const initData = () => {
  ...
  if (!localStorage.getItem('wasteo_bin_data')) {
    setStorageItem('wasteo_bin_data', [
      { binId: 'BIN-A-1', block: 'A', level: 80 },
      { binId: 'BIN-B-1', block: 'B', level: 40 },
      { binId: 'BIN-C-1', block: 'C', level: 20 },
      { binId: 'BIN-D-1', block: 'D', level: 95 },
      { binId: 'BIN-A-2', block: 'A', level: 15 }
    ]);
  }
};
...
export const getComplaintById = async (id) => {
  ...
  const complaint = complaints.find(c => c._id === id || c.complaint_id === id);
...
export const submitComplaint = async (payload) => {
  ...
  if (payload instanceof FormData) {
    newComplaint = {
      ...
      waste_type: payload.get('wasteType') || payload.get('waste_type'),
...
export const updateComplaintStatus = async (id, payload) => {
  const complaints = getStorageItem('wasteo_complaints', []);
  const index = complaints.findIndex(c => c._id === id || c.complaint_id === id);
...
export const getDashboardStats = async () => {
  ...
  // Mock block performance based on demo data
  const blockPerformance = [
    { _id: 'A', count: orders.filter(o => o.status === 'delivered' && o.block === 'A').length },
    { _id: 'B', count: orders.filter(o => o.status === 'delivered' && o.block === 'B').length },
    { _id: 'C', count: orders.filter(o => o.status === 'delivered' && o.block === 'C').length },
    { _id: 'D', count: orders.filter(o => o.status === 'delivered' && o.block === 'D').length },
    { _id: 'E', count: orders.filter(o => o.status === 'delivered' && o.block === 'E').length }
  ];
...
export const getNotifications = async () => {
  ...
  // Map to match frontend expectations (id and isRead)
...
export const markNotificationRead = async (id) => {
  ...
  if (index !== -1) {
    ...
    notifications[index].read = true; // Set both for safety
